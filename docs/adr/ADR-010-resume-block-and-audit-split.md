...
- A run that ends incomplete never reaches the Cleanup Commit, so `.ai/` survives intact on the branch and the failure detail is preserved with no extra mechanism.
...
- Splitting audit out of the read path means audit content must be written *somewhere durable*: `HISTORY.md` in `.ai/` (preserved in branch history by the Cleanup Commit) and successful evidence in commit messages, which survive it.
