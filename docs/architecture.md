...
All four now live under a single `.harness/` container in the consumer repository, except `PRD.md`, which stays at the root because it is the only artifact a human authors ([ADR-014](./adr/ADR-014-single-harness-directory.md)). Nesting them does **not** relax the rule below: each lifecycle keeps its own subdirectory, so every folder-level operation still targets a child, never the shared parent.
...
run.ps1 ──► Iteration 1: BOOTSTRAP (no .harness/run/ exists → this invocation bootstraps)
    │         reads PRD + repo + human docs → generates .harness/knowledge/, .harness/run/ (DoD, PLAN, TASKS, STATE)
...
4. `.harness/knowledge/` (cache of the codebase; loses to it, gets corrected)
5. `.harness/run/` state (own memory)
