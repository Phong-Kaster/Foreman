...
| **Sub-agents** | Separate maker from checker | Three distinct minds (§8), plus **Workers** for parallel implementation. Their limits are harness-enforced via `.loop/agents/`, materialized into `.claude/agents/` each iteration as a build artifact and deny-listed against the engine's own edits: omitting `Bash` from a Worker's `tools` is what makes "no git, no build, no test" real rather than advisory |
