...
The vocabulary of Foreman: a portable execution engine, installed into consumer repositories either as a Claude Code skill or by copying `.loop/` directly, that turns a feature PRD into verified working software with minimal human intervention during coding.
...
**Consumer repository**:
Any repository that installs the loop — either via the Skill (`npx skills@latest add`, then `/foreman`) or by copying `.loop/` directly — and provides a Goal. Foreman never knows the consumer's tech stack.
